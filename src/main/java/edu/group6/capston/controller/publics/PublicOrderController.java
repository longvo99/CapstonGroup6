...
<<<<<<< HEAD
	 ...
	 * 
	 * Users user = (Users) request.getSession().getAttribute("userSession"); if
	 * (user != null) { if (user.getContactAddress() == null) {
	 ...
	 * productId = cookie.getName().substring(0, cookie.getName().lastIndexOf("-"));
	 * if (!cookie.getValue().equals("")) { OrderDTO product = null; if
	 * (productId.substring(0, 1).equals("c")) { String comboId =
	 ...
	 * 
	 * listOrderDTO.add(order); totalCart += total; // remote cookie Cookie
	 * cookieDel = new Cookie(cookie.getName(), ""); cookieDel.setMaxAge(0);
	 * response.addCookie(cookieDel); } } }
	 * 
	 ...
	 * "redirect:/public/checkout"; }
	 * 
	 * for (OrderDTO orderDTO : listOrderDTO) { OrderDetail orderDetail = null; if
	 * (orderDTO.getProductId().contains("c")) { int id =
...
>>>>>>> 04e3d9122beae0954aa2f5e4acb65046986a5ef1
