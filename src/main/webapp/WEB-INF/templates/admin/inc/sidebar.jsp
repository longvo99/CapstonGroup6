...
<link
	...
	rel="stylesheet">
<link
	href="${pageContext.request.contextPath}/resources/admin/assets/vendor/datatables/dataTables.bootstrap4.min.css"
	rel="stylesheet">
		...
		<ul class="navbar-nav sidebar sidebar-light accordion"
			...
			<a
				...
				<div class="sidebar-brand-icon">
					<img
						src="${pageContext.request.contextPath}/resources/admin/assets/img/logo/logo3.png"
						width="50px">
				</div>
			...
			<li class="nav-item active"><a class="nav-link"
				href="${urlHomePage}"> <i class="fas fa-fw fa-tachometer-alt"></i>
					<span>Trang chủ</span>
			</a></li>
			<hr class="sidebar-divider">
			<div class="sidebar-heading">ĐẶC TRƯNG</div>
			<li class="nav-item"><a class="nav-link collapsed" href="#"
				data-toggle="collapse" data-target="#collapseBootstrap"
				aria-expanded="true" aria-controls="collapseBootstrap"> <i
					class="far fa-fw fa-window-maximize"></i> <span>Quản lý địa
						điểm</span>
			</a>
				<div id="collapseBootstrap" class="collapse"
					...
					<div class="bg-white py-2 collapse-inner rounded">
						...
						<c:set var="urlL"
							value="${pageContext.request.contextPath}/public/location/index" />
						<c:if
							test="${sessionScope.userSession.role.roleId ne 'CUSTOMER' }">
							<c:set var="urlL"
								value="${pageContext.request.contextPath}/admin/location/index" />
						</c:if>
						<a class="collapse-item" href="${urlL}">Địa điểm</a>
						<c:if test="${empty sessionScope.userSession}">
							<c:if test="${userDetail.user.role.roleId eq 'ADMIN'}">
								<a class="collapse-item"
									href="${pageContext.request.contextPath}/admin/locationType/index">Kiểu
									địa điểm</a>
								<a class="collapse-item"
									href="${pageContext.request.contextPath}/admin/locationCategory/index">Phân
									loại địa điểm</a>
							</c:if>
						</c:if>
					</div>
			...
			<c:if test="${empty sessionScope.userSession}">
				<c:if test="${userDetail.user.role.roleId eq 'ADMIN'}">
					<li class="nav-item"><a class="nav-link"
						href="${pageContext.request.contextPath}/admin/order/index"> <i
							class="fab fa-fw fa-wpforms"></i> <span>Quản lý đặt hàng</span>
					</a></li>
				</c:if>
			</c:if>
			<li class="nav-item"><c:set var="urlD"
					value="${pageContext.request.contextPath}/public/discount/index" />
				<c:if test="${sessionScope.userSession.role.roleId ne 'CUSTOMER' }">
					<c:set var="urlD"
						value="${pageContext.request.contextPath}/admin/discount/index" />
				</c:if> <a class="nav-link" href="${urlD}"> <i
					class="fab fa-fw fa-wpforms"></i> <span>Quản lý giảm giá</span>
			</a></li>
			<c:if test="${sessionScope.userSession.role.roleId ne 'CUSTOMER' }">
				<li class="nav-item"><a class="nav-link"
					href="${pageContext.request.contextPath}/admin/contact/index">
						<i class="fab fa-fw fa-wpforms"></i> <span>Quản lý liên hệ</span>
				</a></li>
				<c:if test="${empty sessionScope.userSession}">
					<c:if test="${userDetail.user.role.roleId eq 'ADMIN'}">
						<li class="nav-item"><a class="nav-link"
				...
				<li class="nav-item"><a class="nav-link collapsed" href="#"
					...
					aria-expanded="true" aria-controls="collapsePage"> <i
						class="fas fa-fw fa-columns"></i> <span>Quản lý Thông tin</span>
				</a>
									...
									<c:forEach items="${sessionScope.roleList}" var="role">
										<c:choose>
											<c:when test="${userDetail.user.role.roleId eq role.roleId}">
												<a class="collapse-item"
													href="${pageContext.request.contextPath}/admin/user/${role.roleId}">Quản
													lý hồ sơ cá nhân</a>
											</c:when>
											<c:otherwise>
												<a class="collapse-item"
													href="${pageContext.request.contextPath}/admin/user/${role.roleId}">${role.roleName}</a>
											</c:otherwise>
										</c:choose>
									</c:forEach>
									<a class="collapse-item"
										href="${pageContext.request.contextPath}/admin/role/index">Quản
										lý Chức vụ</a>
								</c:if>
