						...
						<div class="card-body">
							...
							<form role="form" action="${actionUrl}" method="post" id="form"
								...
								<div class="form-group">
									<label for="productComboId">ID</label> <input
										class="form-control mb-3" type="text"
										value="${productCombo.productComboId}" id="productComboId"
										name="productComboId" readonly>
								</div>
								<div class="form-group">
									<label for="comboName">Tên Combo</label> <input ${readonly}
										class="form-control mb-3" type="text"
								...
								<div class="form-group">
									<label for="description">Mô tả</label> <input ${readonly}
										class="form-control mb-3" type="text"
										value="${productCombo.description}" id="description"
										name="description">
								</div>
								<div class="form-group">
									<label for="">Chọn sản phẩm: </label> <input ${readonly}
										...
										<ul id="myList">
											<c:if test="${not empty productListChecked}">
												<c:forEach var="product" items="${productListChecked}">
													<li><input ${disabled} checked="checked"
														type="checkbox" class="checkbox" name="check[]"
														value="${product.productId}" style="zoom: 1.5;">
														${product.name};</li>
												</c:forEach>
											</c:if>
...
<<<<<<< Updated upstream
								...
								<input type="hidden" value="${productCombo.location.locationId}"
										...
										<c:if test="${userDetail.user.role.roleId ne 'ADMIN'}">
											...
											<div class="input-group my-3">
												<input type="text" class="form-control"
													value="${productCombo.imagePath}" name="image" disabled
	                    ...
	                    <select class="select2-multiple form-control" name="check[]" multiple="multiple"
	                      id="select2Multiple">
	                       ...
	                       <c:forEach var="product" items="${productListByLocationId}">
		                       <c:choose>
		                       		<c:when test="${fn:contains(productListByProductComboId, product)}">
		                       			<option selected="selected" value="${product.productId}">${product.name}</option>
		                       		</c:when>
		                       		<c:otherwise>
