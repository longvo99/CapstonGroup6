...
<c:choose>
	<c:when test="${not empty sessionScope.userSession}">
		...
		<c:set var="editUrl"
			value="${pageContext.request.contextPath}/public/discount/edit" />
		<c:set var="deleteUrl"
			value="${pageContext.request.contextPath}/public/discount/del" />
	</c:when>
	<c:otherwise>
		<c:set var="addUrl"
		...
		<c:set var="deleteUrl"
			value="${pageContext.request.contextPath}/admin/discount/del" />
	</c:otherwise>
</c:choose>
				...
				<div
					...
					<div style="margin-right: 40px;">
						<a href="javascript:void(0)" id="btnDel"
							class="btn btn-danger btn-md"><i class="fas fa-trash">
								Xóa </i></a>
					</div>
				</div>
								...
								<c:forEach items="${discountList}" var="discount">
									<tr class="odd gradeX">
										...
										<td class="center text-center"><a href=""
											data-toggle="modal"
											data-target="#exampleModalCenter${discount.discountId}"
											class="btn btn-sm btn-success" title="CHI TIẾT"><i
												class="fas fa-info-circle"></i></a> <c:if
												test="${empty sessionScope.userSession}">
												<c:if
													test="${userDetail.user.role.roleId eq discount.users.role.roleId}">
													<a href="${editUrl}/${discount.discountId}"
										...
										<td class="center text-center"><c:if
												test="${empty sessionScope.userSession}">
												<c:if
													test="${userDetail.user.role.roleId eq discount.users.role.roleId}">
													<input type="checkbox" class="checkbox" name="check[]"
														id="customCheck1" value="${discount.discountId}"
														style="zoom: 1.5;">
												</c:if>
											</c:if> <c:if test="${not empty sessionScope.userSession}">
												<input type="checkbox" class="checkbox" name="check[]"
										...
										<div class="modal-dialog modal-dialog-centered"
											...
											<div class="modal-content">
												<div class="modal-header">
													<h5 class="modal-title" id="exampleModalCenterTitle">
														Chi tiết giảm giá: <strong></strong>
													</h5>
													...
													<div class="comment-form">
														<form action="${contextPath}/edit" method="POST">
															<input disabled="disabled" class="form-control mb-3"
																type="hidden" value="${discount.discountId}">
															<c:if test="${empty sessionScope.userSession}">
																<c:if test="${userDetail.user.role.roleId ne 'ADMIN'}">
																	<div class="form-group">
																		<label for="name">Tên nhà hàng</label> <input
																			disabled="disabled" class="form-control mb-3"
																			type="text" value="${discount.location.locationName}">
																	</div>
																</c:if>
															</c:if>
															<c:if test="${not empty sessionScope.userSession}">
															...
															<div class="form-group">
																<label for="name">Tiêu đề</label> <input
																	disabled="disabled" class="form-control mb-3"
																	type="text" value="${discount.title}">
															</div>
															<div class="form-group">
					                      ...
					                      <input disabled="disabled" class="form-control mb-3" type="text" value="${discount.code}">
															...
															<c:if test="${empty sessionScope.userSession}">
																<c:if test="${userDetail.user.role.roleId ne 'ADMIN'}">
																	<div class="form-group">
																		<label for="opentime">Điều kiện giảm</label> <input
																			disabled="disabled" class="form-control mb-3"
																			type="text"
															...
															<div class="form-group">
																<label for="locationcategory">Ngày bắt đầu</label> <input
																	disabled="disabled" class="form-control mb-3"
																	type="text" value="${discount.startDate}">
															</div>
															<div class="form-group">
																<label for="locationtype">Ngày kết thúc</label> <input
																	disabled="disabled" class="form-control mb-3"
																	type="text" value="${discount.endDate}">
															</div>
															<div class="form-group">
																<label for="locationtype">Người tạo</label> <input
																	disabled="disabled" class="form-control mb-3"
																	type="text" value="${discount.users.username}">
															</div>
															<div class="form-group">
																<label for="locationtype">Giới hạn dùng (lần)</label> <input
																	disabled="disabled" class="form-control mb-3"
																	type="text" value="${discount.limitedUse}">
															</div>
															<div class="form-group">
																<label for="locationtype">Số lần dùng/Người</label> <input
																	disabled="disabled" class="form-control mb-3"
																	type="text" value="${discount.limitedPerUser}">
															</div>
															<div class="form-group">
																<label>Ảnh</label>
