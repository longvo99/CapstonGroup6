...
<c:choose>
	<c:when test="${not empty sessionScope.userSession}">
		<c:set var="addUrl"
			value="${pageContext.request.contextPath}/public/location/add" />
		<c:set var="editUrl"
			value="${pageContext.request.contextPath}/public/location/edit" />
		<c:set var="imageUrl"
			value="${pageContext.request.contextPath}/public/location/image" />
		<c:set var="deleteUrl"
			value="${pageContext.request.contextPath}/public/location/delete" />
		<c:set var="productUrl"
			value="${pageContext.request.contextPath}/public/product/index" />
		<c:set var="comboUrl"
			value="${pageContext.request.contextPath}/public/combo/index" />
	</c:when>
	<c:otherwise>
		<c:set var="addUrl"
			value="${pageContext.request.contextPath}/admin/location/add" />
		<c:set var="editUrl"
			value="${pageContext.request.contextPath}/admin/location/edit" />
		<c:set var="imageUrl"
			value="${pageContext.request.contextPath}/admin/location/image" />
		<c:set var="deleteUrl"
			value="${pageContext.request.contextPath}/admin/location/delete" />
		<c:set var="productUrl"
			value="${pageContext.request.contextPath}/admin/product/index" />
		<c:set var="comboUrl"
			value="${pageContext.request.contextPath}/admin/combo/index" />
	</c:otherwise>
</c:choose>
					...
					<div class="col-sm-10">
						<c:if test="${empty sessionScope.userSession}">
							<c:if test="${userDetail.user.role.roleId ne 'ADMIN'}">
								<a href="${addUrl}" class="btn btn-success btn-md"><i
								...
								<c:forEach items="${locationList}" var="location">
									<tr class="odd gradeX">
										...
										<td class="center text-center"><a
											href="${editUrl}/${location.locationId}"
											class="btn btn-sm btn-success" title="CHI TIẾT"><i
												class="fas fa-info-circle"></i></a> <c:if
												test="${empty sessionScope.userSession}">
												<c:if test="${userDetail.user.role.roleId ne 'ADMIN'}">
													<a
														href="${pageContext.request.contextPath}/admin/location/image/${location.locationId}"
														title="HÌNH ẢNH" class="btn btn-sm btn-primary"><i
														class="fas fa-image" aria-hidden="true"></i></a>
												</c:if>
											</c:if> <a href="${productUrl}/${location.locationId}"
											class="btn btn-sm btn-warning" title="SẢN PHẨM"><i
												class="fab fa-product-hunt"></i></a> <a
											href="${comboUrl}/${location.locationId}"
											class="btn btn-sm btn-info" title="COMBO"><i
												class="fas fa-certificate"></i></a> <a href="${deleteUrl}"
											class="btn btn-danger btn-sm"
											onclick="return confirm('Bạn có chắc muốn xóa không');"
											title="XÓA"><i class="fas fa-trash"></i></a> <c:if
												test="${empty sessionScope.userSession}">
												<c:if test="${userDetail.user.role.roleId eq 'ADMIN'}">
													<a href="" data-toggle="modal"
														data-target="#exampleModalCenter${location.locationId}"
														title="LIÊN HỆ" class="btn btn-sm btn-info"><i
														class="fas fa-comment-dots"></i></a>
													<!-- Modal Center -->
													<div class="modal fade"
														id="exampleModalCenter${location.locationId}"
															...
															<div class="modal-content">
																<div class="modal-header">
																	<h5 class="modal-title" id="exampleModalCenterTitle">
																		Gửi liên hệ cho: <strong>${location.locationName}</strong>
																	</h5>
																...
																<div class="modal-body">
																	<div class="comment-form">
																		<form role="form"
																			action="${pageContext.request.contextPath}/mail"
																			method="post">
																			<div class="form-group form-inline">
																				<div class="form-group col-lg-6 col-md-12 name">
																					<input type="text" class="form-control"
																						name="tieude" id="tieude" placeholder="Tiêu đề"
																			...
																			<div class="form-group">
																				<textarea class="form-control mb-10" id="message"
																					rows="5" name="message" placeholder="Nội dung"
																					onfocus="this.placeholder = ''"
