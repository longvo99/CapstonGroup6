					...
					<div class="card mb-4">
						<div class="card-body">
							<form action="${pageContext.request.contextPath}/admin/user/add"
								role="form" method="post" name="form" id="form">
								...
								<div class="form-group">
									<label for="contactEmail">Email</label> <input
										class="form-control mb-3" type="text"
										value="${users.contactEmail}" id="contactEmail"
										name="contactEmail">
								</div>
								<div class="form-group">
									<label>Chức vụ</label> <select class="form-control" id="role"
										name="role.roleId">
										<c:if test="${not empty roleList}">
											<c:forEach items="${roleList}" var="role">
												<c:if
													test="${role.roleId ne 'CUSTOMER' && role.roleId ne 'ADMIN'}">
