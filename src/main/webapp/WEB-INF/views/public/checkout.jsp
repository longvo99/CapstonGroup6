...
<section class="final-order section-padding bg-light-theme">
	<div class="container-fluid">
		<div class="row">
			<div class="col-lg-7">
				<form
					action="${pageContext.request.contextPath}/public/checkout/cash"
					id="formId" method="POST">
					<div class="main-box padding-20">
						<div class="row mb-xl-20">
							<div class="col-md-10">
								<div class="section-header-left">
									<h3 class="text-light-black header-title fw-700">Xác nhận
										đơn hàng</h3>
								</div>
								<h6 class="text-light-black fw-700 fs-16">Xác nhận địa chỉ,
									số điện thoại, thông tin thanh toán</h6>
								...
								<div style="width: 300px; display: inline-block;">
									<div class="form-group">
										<label class="text-light-white fw-700" for="district">Quận
											huyện</label> <select name="district" id="state"
								...
								<div style="width: 300px;">
									<div class="form-group">
										<label class="text-light-white fw-700">Số điện thoại</label> <input
											value="${sessionScope.userSession.contactPhone}" type="text"
											name="phone" class="form-control form-control-submit">
									</div>
								</div>
								<div class="section-header-left">
								...
								<div class="form-group">
									<textarea class="form-control form-control-submit" name="note"
										style="width: 610px;" rows="4"></textarea>
								</div>
							</div>
						</div>
						<div class="row">
							<div class="col-12">
								<c:if test="${ not empty listDiscount}">
									<div class="payment-sec">
										<div class="section-header-left">
											<h3 class="text-light-black header-title">Sử dụng mã
												giảm giá</h3>
										</div>
									</div>
									<div class="col-lg-12">
												...
												<div class="restaurent-product-detail">
													...
													<div class="restaurent-product-left">
														<div class="restaurent-product-title-box">
															<div class="restaurent-product-box">
																...
																<div class="restaurent-product-label">
																	<span
																		class="rectangle-tag bg-gradient-red text-custom-white"><fmt:formatNumber
																			value="${discount.rateDiscount}" type="number" />${rateDiscountA}</span>
																	<!-- <span class="rectangle-tag bg-dark text-custom-white">Combo</span> -->
																</div>
																<div style="margin-left: 10px;"
																	class="restaurent-product-title">
																	<h6 class="mb-2" data-toggle="modal"
																		data-target="#restaurent-combo">
																		<a href="javascript:void(0)"
																			class="text-light-black fw-600">Điều
																			kiện:${discount.discountRule.ruleContent} tối thiểu
																			là : ${discount.valueRule}</a>
																	</h6>
														...
														<div class="restaurent-product-caption-box">
															<span class="text-light-white">Số lần dùng còn
																lại: ${discount.limitedPerUser}</span>
														</div>
														<div class="restaurent-tags-price">
															<div class="restaurent-product-price">
																<c:if test="${discount.discountRule.ruleId eq 2}">
																	<c:choose>
																		<c:when
																			test="${totalOrderedPrice >= discount.valueRule and totalCart >= discount.valueRule}">
																			<button type="button"
																				id="button${discount.discountId}"
																				onclick="applyDiscount(${discount.rateDiscount},${discount.discountId})"
																				class="btn btn-primary btn-sm shadow-none">Áp
																				dụng</button>
																		</c:when>
																		<c:otherwise>
																			<button disabled="disabled"
																				class="btn btn-danger btn-sm shadow-none">Áp
																				dụng</button>
																		</c:otherwise>
																	</c:choose>
																</c:if>
																<c:if test="${discount.discountRule.ruleId eq 1}">
																	<c:choose>
																		<c:when
																			test="${totalCart >= discount.valueRule and totalCart >= discount.valueRule}">
																			<button type="button"
																				id="button${discount.discountId}"
																				onclick="applyDiscount(${discount.rateDiscount}, ${discount.discountId})"
																				class="btn btn-primary btn-sm shadow-none">Áp
											...
											<div class="payment-option-tab">
												<ul class="nav nav-tabs">
													...
													<li class="nav-item"><a class="nav-link fw-600 active"
														data-toggle="tab" id="payCash" href="#cash">Tiền mặt</a></li>
													<li class="nav-item"><a class="nav-link fw-600"
														data-toggle="tab" id="payPaypal" href="#paypal">PayPal<sup>TM</sup></a>
													</li>
												...
												<div class="tab-content">
													<div class="tab-pane active" id="cash">
														<p class="text-light-black">Chuẩn bị sẵn tiền mặt khi
															bạn nhận được đơn đặt hàng của mình.</p>
														<div class="form-group">
															<button type="submit"
																class="btn-first green-btn text-custom-white full-width fw-500">Đặt
																hàng</button>
														</div>
													</div>
			...
			<div class="col-lg-5">
				<div class="sidebar">
					<div class="cart-detail-box">
						<div class="card">
							<div class="card-header padding-15 fw-700">Chi tiết đơn
								hàng</div>
										...
										<c:when test="${not empty listOrderDTO}">
											<c:forEach items="${listOrderDTO}" var="orderDTO">
												<div class="cat-product"
													id="cat-product${orderDTO.productId}"
													style="box-sizing: border-box;">
													<div class="quantity">
														<div class="append"
															style="float: left; margin-right: 10px;">
															<button class="minus-btn"
																onclick="minusQuantity('${orderDTO.productId}', ${orderDTO.price})"
														...
														<input type="text"
															...
															class="text-center" name="name"
															id="quantity${orderDTO.productId}"
															value="${orderDTO.quantity}" readonly="readonly">
														<div class="prepend" style="float: left;">
															<button class="plus-btn"
																onclick="plusQuantity('${orderDTO.productId}', ${orderDTO.price})"
																type="button" name="button">
													...
													<div class="cat-name">
														<a href="#">
															<p class="text-light-green fw-700">${orderDTO.name}</p> <span
															class="text-light-white fw-700">size L</span>
														</a>
													</div>
													...
													<div class="price">
														<a href="#" id="totalPriceProduct${orderDTO.productId}"
															class="text-dark-white fw-500"><fmt:formatNumber
								...
								<c:if test="${not empty listOrderDTO}">
									<div class="item-total">
										<div class="total-price border-0 pb-0">
											<span class="text-dark-white fw-600">Phí ship:</span> <span
												class="text-dark-white fw-600">+ 20,000đ</span>
										</div>
										<div id="saleRateDiscount"
											class="total-price border-0 pt-0 pb-0"></div>
									</div>
								</c:if>
							</div>
							<c:if test="${not empty listOrderDTO}">
								<div class="card-footer p-0 modify-order">
									<div class="total-amount">
										<span class="text-custom-white fw-700">TỔNG:</span> <span
											id="totalCartCheckOut" class="text-custom-white fw-700"><fmt:formatNumber
												value="${totalCart + 20000}" type="number" /></span>
									</div>
								</div>
							</c:if>
						</div>
