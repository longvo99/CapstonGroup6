			...
			<div class="col-md-12">
				<div class="tracking-sec">
					<div class="tracking-details padding-20 p-relative">
						<h5 class="text-light-black fw-600">Mã đơn hàng:
							${order.orderId} - Địa điểm: ${order.location.locationName}</h5>
						...
						<div id="add-listing-tab" class="step-app">
								...
								<c:if test="${order.orderStatus.orderStatusId eq 6}">
									<c:forEach items="${orderStatusList}" var="orderStatus">
										<c:if test="${orderStatus.orderStatusId eq 6}">
											<li class="done active"><a href="javascript:void(0)">
													<span class="number"></span> <span class="step-name">${orderStatus.statusName}</span>
											</a></li>
										</c:if>
									</c:forEach>
								</c:if>
								<c:if test="${order.orderStatus.orderStatusId ne 6}">
									<c:forEach items="${orderStatusList}" var="orderStatus">
										<c:choose>
												...
												<c:choose>
													<c:when test="${orderStatus.orderStatusId eq 2}">
														<li class='active'><a href="javascript:void(0)">
																<span class="number"></span> <span class="step-name">${orderStatus.statusName}<!-- <br>8:38pm --></span>
														</a></li>
													</c:when>
													<c:when
														test="${orderStatus.orderStatusId > 2 and orderStatus.orderStatusId ne 6}">
														<li><a href="javascript:void(0)"> <span
																class="number"></span> <span class="step-name">${orderStatus.statusName}<!-- <br>8:38pm --></span>
														</a></li>
													</c:when>
												</c:choose>
											</c:when>
											<c:otherwise>
												<c:if
													test="${orderStatus.orderStatusId ne 2 and orderStatus.orderStatusId < 6}">
													<c:set var="classDone" value="" />
													<c:choose>
														<c:when
															test="${orderStatus.orderStatusId < order.orderStatus.orderStatusId}">
															<c:set var="classDone" value="class='done'" />
														</c:when>
														<c:when
															test="${orderStatus.orderStatusId eq order.orderStatus.orderStatusId}">
															<c:set var="classDone" value="class='active'" />
														</c:when>
													</c:choose>
													<li ${classDone}><a href="javascript:void(0)"> <span
															class="number"></span> <span class="step-name">${orderStatus.statusName}<!-- <br>8:38pm --></span>
													</a></li>
												</c:if>
											</c:otherwise>
							...
							<c:if test="${order.orderStatus.orderStatusId < 3}">
								<div style="display: inline-block;" class="">
									<a
										href="${pageContext.request.contextPath}/public/deleteOrder/${order.orderId}"
										class="btn-first white-btn fw-600 help-btn">Hủy đơn hàng</a>
								</div>
							</c:if>
						</div>
					</div>
						...
						<div class="row">
							<div class="col-lg-4">
								<div class="recipt-name full-width padding-tb-10 pt-0">
									...
									<span class="text-light-white ">${user.contactName}</span> <span
										class="text-light-white ">Địa chỉ</span> <span
										class="text-light-white ">${userAddress.stress}</span> <span
										class="text-light-white " id="resultAdress"></span>
							...
							<div class="col-lg-4">
								<div class="ad-banner padding-tb-10 h-100">
									<img src="https://via.placeholder.com/337x139"
										...
										<div class="img-name-value">
											<div class="product-img">
												<a href="#"> <img style="width: 50px; height: 50px;"
													src="${pageContext.request.contextPath}/resources/admin/assets/img/uploads/${orderDetail.imagePath}"
													alt="#">
												</a>
											</div>
											<div class="product-value">
												<span class="text-light-white">${orderDetail.quantity}</span>
											</div>
											<div class="product-name">
												<span><a href="#" class="text-light-white">${orderDetail.name}</a></span>
											</div>
										</div>
										<div class="price">
											<span class="text-light-white">${orderDetail.price * orderDetail.quantity}</span>
										</div>
									</div>
									<c:set var="total"
										value="${total + (orderDetail.price * orderDetail.quantity)}" />
								</c:forEach>
							</div>
						</div>
					</div>
						...
						<div class="col-lg-7">
							<div class="payment-method mb-md-40">
								...
								<div class="method-type">
									<i class="far fa-credit-card text-dark-white"></i> <span
										class="text-light-white">Trả tiền mặt</span>
								</div>
							</div>
						</div>
						<div class="col-lg-5">
							<div class="price-table u-line">
								...
								<div class="item">
									<span class="text-light-white">Phí ship:</span> <span
										class="text-light-white">20,000đ</span>
								</div>
							</div>
							<div class="total-price padding-tb-10">
								<h5 class="title text-light-black fw-700">
									Tổng: <span>${total + 20000}</span>
								</h5>
							</div>
						</div>
					</div>
												...
												<c:forEach items="${listOrder}" var="order">
													<tr class="row100 body">
														...
														<td class="cell100 column4"><a
															href="${pageContext.request.contextPath}/public/orderdetails/${order.orderId}"><i
					...
					<div class="row">
						<div class="col-12">
							<div class="form-group">
								<label class="text-light-white fs-14">E-mail</label> <input
									type="email" name="#" class="form-control form-control-submit"
									placeholder="E-mail ID">
							</div>
							<div class="form-group">
								<label class="text-light-white fs-14">ZIP Code</label> <input
									type="number" name="#" class="form-control form-control-submit"
									placeholder="10421">
							</div>
							<div class="form-group">
								<button type="submit" class="btn-second btn-submit full-width">Save
									$12 on your first order</button>
							</div>
							<div class="form-group">
								<button type="submit"
									class="btn-second btn-default full-width text-light-green fw-700"
									data-dismiss="modal">Dismiss</button>
							</div>
							<div class="text-center">
								<span class="text-light-black fs-12">*Valid on first
									order only, for one-time use, subject to foodmartâs
									verification. Additional terms may apply. By signing up, you
									agree to receive marketing and promotional emails and
				...
				<div class="col-lg-3">
					<div class="modal-sidebar padding-20">
						<div class="content-box padding-15 u-line">
							<h5 class="text-light-black title fw-600 no-margin">
								Delivered <span><a href="#" class="fs-14">Help</a></span>
							</h5>
							<p class="text-light-white mb-xl-20">Five Guys 8:56pm arrival</p>
							<div id="add-listing-tab1" class="step-app mb-xl-20">
								<ul class="step-steps">
									<li class="done order-note"><a href="javascript:void(0)">
											<span class="number"></span>
									</a></li>
									<li class="active make-order"><a href="javascript:void(0)">
											<span class="number"></span>
									</a></li>
									<li class="bike"><a href="javascript:void(0)"> <span
											class="number"></span>
									</a></li>
									<li class="home-delivery"><a href="javascript:void(0)">
											<span class="number"></span>
									</a></li>
								</ul>
							</div>
							<div class="rating-box">
								<span class="text-light-black fs-12">Rate your delivery</span>
								<div class="ratings">
									<span class="text-light-white"> <i class="fas fa-star"></i>
									</span> <span class="text-light-white"> <i class="fas fa-star"></i>
									</span> <span class="text-light-white"> <i class="fas fa-star"></i>
									</span> <span class="text-light-white"> <i class="fas fa-star"></i>
									</span> <span class="text-light-white"> <i class="fas fa-star"></i>
									</span> <span class="arrow"><a href="#"><i
											class="fas fa-chevron-right"></i></a></span>
								</div>
							</div>
						</div>
						<div class="content-box padding-15 u-line">
							<h6 class="text-light-black fw-500 mb-2 title">
								Five Guys <span><a href="#" class="fs-12"><i
										class="fas fa-chevron-right"></i></a></span>
							</h6>
							<p class="text-light-black mb-1">2 Items</p>
							<p class="text-light-white no-margin fs-12">1x Little
								Hamburger</p>
							<p class="text-light-white no-margin fs-12">1x Hamburger</p>
						</div>
						<div class="content-box padding-15 u-line">
							<h6 class="text-light-black fw-500 mb-2 title">Delivery
								Address</h6>
							<p class="text-light-black no-margin">314 79th St, Brooklyn,
								NY 11209, USA</p>
						</div>
						<button class="md-close btn-second btn-submit full-width">
